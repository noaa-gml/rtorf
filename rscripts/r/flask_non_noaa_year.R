#example Rscript rscripts/flask_year.R 2020 obs/obs_flask.csv
...
out <- paste0("master/master_flask_no_noaa_", yy)
...
# max altitude can vary across site over time
# before filtering, the max altitude for site WSA was 25,
# but after the filter3
dfa <- df[,
          max(altitude_final),
          by = site_code] |> unique()
...
# 9 Filter out all non NOAA measurements
unique(df3$lab_1_abbr)
df3 <- df3[lab_1_abbr != "NOAA"]

# 10 Save in master ####
cat(paste0("Saving in master/ \n"))
master <- df3
...
# 10 Save csvy in master ####
cat(paste0("Saving csvy in master/ \n"))
notes <- c("sector: flask no NOAA",
