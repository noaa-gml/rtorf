...
```{r sfsave, echo = FALSE, message=FALSE}
library(sf)
sdft <- st_as_sf(dft[stat == "mean"], 
                 coords = c("longitude", "latitude"), 
                 crs = 4326)
sdft$value <- sdft$value*1e09
png("man/figures/plotsf.png", width = 2000, height = 1500, res = 200)
plot(sdft["value"], 
     reset = FALSE, 
     axes = TRUE, 
     graticule = TRUE,
...
```{r sf, echo = TRUE, message=FALSE, eval = FALSE}
library(sf)
sdft <- st_as_sf(dft[stat == "mean"], 
                 coords = c("longitude", "latitude"), 
                 crs = 4326)
...
```{r plot1, tidy = TRUE, dpi=300, fig.cap="First two sites in ObsPack"}
